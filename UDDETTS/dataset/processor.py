...
def padding(data, use_spk_embedding, mode='train', gan=False):
    ...
    for sample in data:
        ...
        emo_id = torch.stack([torch.tensor(sample[i]['emo_id'], dtype=torch.int32) for i in order], dim=0)
        ADV = torch.stack([torch.tensor(sample[i]['ADV'], dtype=torch.float) for i in order], dim=0)
        batch = {
            "utts": utts,
            "speech": speech,
