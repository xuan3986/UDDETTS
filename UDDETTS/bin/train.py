...
from UDDETTS.utils.executor import Executor
from UDDETTS.utils.train_utils import (
    init_distributed,
    init_model,
...
def main():
    ...
    # Do some sanity checks and save config to arg.model_dir
    ...
    # load checkpoint
    model, start_step, start_epoch = init_model(configs, args)
    ...
    # Get executor
    executor = Executor(step = start_step + 1, epoch = start_epoch + 1, gan=gan)

    # Init scaler, used for pytorch amp mixed precision training
    scaler = torch.cuda.amp.GradScaler() if args.use_amp else None
    logging.warning('start step {} start epoch {}'.format(start_step, start_epoch))
    ...
    for epoch in range(start_epoch + 1, info_dict['max_epoch']):
        ...
        dist.barrier()
        group_join = dist.new_group(backend=args.ddp.dist_backend, timeout=datetime.timedelta(seconds=args.timeout))
        if gan is True:
